...
let CACHE = {
    token: {
        LAST: ""
    }, // tokenAddress => TokenBasic object
    tokenHistory: {}
}; 
...
async function updatePairPriceWithReserves( token0, token1 ){
    ...
    let tokenHistory = CACHE.tokenHistory[dependantToken.contract];
...
async function getTokenInfos( token ){
    ...
    if(tokenInfo && !tokenInfo.burned)
        tokenInfo.burned = (await getBurnedAmount(token))/10**tokenInfo.decimals;
    return tokenInfo;
}
